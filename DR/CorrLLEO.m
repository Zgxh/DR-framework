function [mappedX,err]=CorrLLEO(X, no_dims, k, tol)
eig_impl='matlab';
n = size(X, 1);
% index=randperm(n);
% X=X(index,:);
...
%%
M1=llew([CorrX;X1],no_dims,k,tol);
M2=llew([CorrX;X2],no_dims,k,tol);
...
root=strcat('.\mat\CLLE_K=',num2str(k));
save(root,'mappedX');

err=sum(eigenvals(2:no_dims + 1));

function [M]=llew(X, no_dims, k,tol)
